...
start_dashboard() {
    ...
    nohup env TODOIST_AGENT_BACKEND="${ai_backend}" TODOIST_DASHBOARD_LLM_BACKEND_LOCK="${ai_backend}" TODOIST_AGENT_MODEL_ID="${MODEL_ID}" setsid uv run uvicorn todoist.web.api:app --host 127.0.0.1 --port "${API_PORT}" </dev/null > "${API_LOG_FILE}" 2>&1 &
    local api_pid="$!"
    echo "${api_pid}" > "${PID_DIR}/api.pid"
    wait_for_process "${api_pid}" "API"

    log_note "Starting observer..."
    nohup env HYDRA_FULL_ERROR=1 TODOIST_AGENT_BACKEND="${ai_backend}" TODOIST_DASHBOARD_LLM_BACKEND_LOCK="${ai_backend}" TODOIST_AGENT_MODEL_ID="${MODEL_ID}" setsid uv run python3 -m todoist.run_observer --config-dir configs --config-name automations </dev/null > "${OBSERVER_LOG_FILE}" 2>&1 &
    local observer_pid="$!"
