...
from todoist.core.utils import Anonymizable
from loguru import logger


class Database(
    Anonymizable, DatabaseActivity, DatabaseProjects, DatabaseTasks, DatabaseLabels
):
        ...
        for cls in type(self).__mro__:
            ...
            if callable(reset_fn):
                ...
                try:
                    ...
                    logger.error(
                        f"Reset failed for {cls.__name__}: {e.__class__.__name__}: {e}"
                    )
    ...
    def anonimizable_subdatabases(self):
        ...
        """
        return list(
            filter(lambda x: hasattr(x, "anonymize_sub_db"), Database.__bases__)
        )
