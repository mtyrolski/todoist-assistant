...
def process_rescheduled_tasks(
    df_activity: pd.DataFrame, active_tasks: list[Task]
) -> pd.DataFrame:
    rescheduled_counts = (
    ...
    rescheduled_tasks = cast(
        pd.DataFrame,
        rescheduled_counts.sort_values(ascending=False).reset_index(
    ...
    recurring_task_names = set(
        task.task_entry.content for task in active_recurring_tasks
    )
