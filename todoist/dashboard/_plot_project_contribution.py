...
def plot_project_contribution_timeline(
    ...
    if event_column not in frame:
        return _empty_figure("No completion events in this period.")
    frame = frame.loc[frame[event_column] == "completed"]
    if frame.empty:
    ...
    frame["timeline_project"] = [
        name if name == root else f"{root} → {name}"
        for root, name in zip(roots, names, strict=False)
    ]
    frame["week"] = frame.index.to_period("W-MON").start_time
    pivot = frame.pivot_table(
        index="timeline_project",
        columns="week",
        aggfunc="size",
        fill_value=0,
    )
    ...
    return go.Figure(
        data=go.Heatmap(
            z=pivot.to_numpy(),
            x=[value.strftime("%d %b") for value in pivot.columns],
            y=pivot.index.tolist(),
            colorscale="Blues",
            colorbar={"title": "Completed"},
            hovertemplate="%{y}<br>Week of %{x}: %{z} completed<extra></extra>",
        ),
        layout={
            "template": "plotly_dark",
            "height": max(340, 58 * len(pivot.index) + 130),
            "margin": {"l": 24, "r": 24, "t": 16, "b": 36},
            "paper_bgcolor": "#111318",
            "plot_bgcolor": "#111318",
            "xaxis": {"title": "Week"},
            "yaxis": {
                "title": "Parent project → subproject",
                "autorange": "reversed",
            },
        },
    )
