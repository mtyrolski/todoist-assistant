...
def extract_metrics(
    df_activity: pd.DataFrame, granularity: str
    ...
    end_range = cast(
        datetime, pd.Timestamp(cast(Any, df_activity.index.max())).to_pydatetime()
    )
    ...
    _get_total_completed_tasks = partial(
        _get_total_tasks_by_type, task_type="completed"
    )
    _get_total_added_tasks = partial(_get_total_tasks_by_type, task_type="added")
    _get_total_rescheduled_tasks = partial(
        _get_total_tasks_by_type, task_type="rescheduled"
    )
    for metric_name, metric_func, inverse in [
        ...
        previous_value = metric_func(
            df_activity, previous_beg_range, previous_end_range
        )
...
def get_badges(active_projects: list[Project]) -> str:
    ...
    badge = (
        f":red-badge[P1 tasks {p1_task_count}🔥] "
