...
class _StubActivity(Activity):
    def __init__(self, name: str, nweeks: int, *, is_long: bool = False):
        super().__init__(name=name, nweeks_window_size=nweeks, early_stop_after_n_windows=1)
...
def test_select_update_env_automations_filters_long_automations_before_activity_selection() -> None:
    ...
    selected = select_update_env_automations([long_regular, short_activity, long_activity, regular])

    assert selected == [short_activity, regular]


def test_run_configured_automations_uses_skip_long_for_run_entrypoint(monkeypatch) -> None:
    ...
    monkeypatch.setattr("todoist.automations.entrypoint.configure_runtime_logging", lambda **kwargs: None)
    monkeypatch.setattr("todoist.automations.entrypoint.Database", lambda path: path)
    monkeypatch.setattr("todoist.automations.entrypoint.hydra.utils.instantiate", lambda value: automations)
    monkeypatch.setattr("todoist.automations.entrypoint.tqdm", lambda value, desc: value)
    ...
    run_configured_automations(config, select_automations=_capture_selected, skip_long=True)
