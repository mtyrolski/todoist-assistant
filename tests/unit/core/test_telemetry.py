...
def test_bootstrap_config_defaults_to_disabled(monkeypatch, tmp_path: Path) -> None:
    ...
    config = json.loads(
        (tmp_path / telemetry.CONFIG_FILENAME).read_text(encoding="utf-8")
    )
...
def test_send_event_skips_when_endpoint_missing(monkeypatch, tmp_path: Path) -> None:
    ...
    sent = telemetry.send_event(
        "install_success", config_dir=tmp_path, _data_dir=tmp_path
    )

    assert sent is False


def test_maybe_send_install_success_writes_sentinel(
