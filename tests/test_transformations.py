...
class TestDataTransformations(unittest.TestCase):
    """Test data transformation operations and dataframe creation."""

    def setUp(self):
        """Set up test fixtures."""
        # Create mock events
        self.event_entry1 = EventEntry(
            id="event1",
            object_type="item",
            object_id="task1",
        ...
        self.event1 = Event(
            event_entry=self.event_entry1,
            id="event1",
            date=dt.datetime(2024, 1, 1, 12, 0, 0)
        )
        
        self.event2 = Event(
            event_entry=self.event_entry2,
            id="event2",
            date=dt.datetime(2024, 1, 2, 14, 30, 0)
        )
        
        # Create mock projects
        self.project_entry1 = ProjectEntry(
            id="project1",
            name="Project One",
            color="blue",
            parent_id=None,
        ...
        self.project_entry2 = ProjectEntry(
            id="project2",
            name="Project Two",
            color="red",
            parent_id=None,
        ...
        self.project1 = Project(
            id="project1",
            project_entry=self.project_entry1,
            tasks=[],
            is_archived=False
        )
        ...
        self.project2 = Project(
            id="project2",
            project_entry=self.project_entry2,
            tasks=[],
            is_archived=False
        )

    def test_events_to_dataframe_basic(self):
        """Test basic events_to_dataframe functionality."""
        activity = {self.event1, self.event2}
        project_id_to_name = {
        ...
        self.assertEqual(event1_row['title'], 'Task 1 Content')
        self.assertEqual(event1_row['type'], 'completed')
        self.assertEqual(event1_row['parent_project_id'], 'project1')
        self.assertEqual(event1_row['parent_project_name'], 'Project One')
        self.assertEqual(event1_row['root_project_id'], 'project1')
        self.assertEqual(event1_row['root_project_name'], 'Project One')
        
        # Check event 2 data
        event2_row = df[df['id'] == 'event2'].iloc[0]
        self.assertEqual(event2_row['title'], 'Task 2 Content')
        self.assertEqual(event2_row['type'], 'added')
        self.assertEqual(event2_row['parent_project_id'], 'project2')
        self.assertEqual(event2_row['parent_project_name'], 'Project Two')

    def test_events_to_dataframe_chronological_order(self):
        ...
        activity = {future_event, self.event1, self.event2}
        project_id_to_name = {"project1": "Project One", "project2": "Project Two"}
        project_id_to_root = {"project1": self.project1, "project2": self.project2}
        ...
        first_row = df.iloc[0]
        self.assertEqual(first_row['id'], 'event1')  # 2024-01-01
        
        # Verify the last row is the newest event
        last_row = df.iloc[-1]
        self.assertEqual(last_row['id'], 'event_future')  # 2024-01-05

    def test_events_to_dataframe_unsupported_event_types(self):
        ...
        activity = {self.event1, unsupported_event}
        project_id_to_name = {"project1": "Project One"}
        project_id_to_root = {"project1": self.project1}
        ...
        with patch('todoist.types.logger') as mock_logger:
            ...
            self.assertEqual(df.iloc[0]['id'], 'event1')
            
            # Should log filtering information
            mock_logger.info.assert_called()

    def test_events_to_dataframe_missing_project_mapping(self):
        ...
        activity = {self.event1, missing_project_event}
        project_id_to_name = {"project1": "Project One"}  # Missing project not included
        project_id_to_root = {"project1": self.project1}   # Missing project not included
    ...
    def test_events_to_dataframe_empty_activity(self):
    ...
    def test_event_name_extraction(self):
        ...
        self.assertEqual(content_event.name, "Task 1 Content")
        ...
        self.assertEqual(name_event.name, "Project Name")
        ...
        self.assertIsNone(no_name_event.name)
    ...
    def test_dataframe_column_types(self):
        ...
        self.assertTrue(df['parent_project_id'].dtype == 'object')
        self.assertTrue(df['root_project_id'].dtype == 'object')
        
        # Check that date column contains datetime objects
        self.assertTrue(all(isinstance(d, dt.datetime) for d in df['date']))
        
        # Check that string columns are strings
        self.assertTrue(df['title'].dtype == 'object')
        self.assertTrue(df['type'].dtype == 'object')
        self.assertTrue(df['parent_project_name'].dtype == 'object')
        self.assertTrue(df['root_project_name'].dtype == 'object')
